...
class DatasetViewModel: ObservableObject {
    ...
    func chooseDirectory() async {
        ...
        if panel.runModal() == .OK, let url = panel.url {
            ...
            // Start filesystem watchers for the selected directory
            startWatching()
        }
    }
    ...
    func restorePreviousDirectoryIfAvailable() async {
        if let bookmarkData = UserDefaults.standard.data(forKey: "lastDatasetFolderBookmark") {
            ...
            do {
                ...
                // Start filesystem watchers for the restored directory
                startWatching()
            } catch {
    ...
    func navigateToFolder(_ url: URL) {
        ...
        // Stop old watchers before changing directory (WATCH-04)
        stopWatching()
        ...
        // Start watchers for the new directory
        startWatching()
    }

    // MARK: - Filesystem Watchdog Lifecycle
    ...
    // MARK: - Filesystem Watchdog Rescan

    private func performContentRescan() {
        // Bounce-back guard: prevent infinite loop if reading directory triggers .write event
        ...
        let oldPairs = pairs
        let oldURLs = Set(oldPairs.map(\.imageURL))

        // If the current directory no longer exists, navigate to surviving ancestor
        ...
        // Capture selected URL before rescan (UUIDs will be regenerated by scanCurrentDirectory)
        let oldSelectedURL = pairs.first(where: { $0.id == selectedID })?.imageURL
        let oldSelectedIndex = pairs.firstIndex(where: { $0.id == selectedID })

        // Rescan: updates self.pairs and sets selectedID = pairs.first?.id
        scanCurrentDirectory()
        ...
        let surviving = oldURLs.intersection(newURLs)

        // Evict removed URLs from cache and cancel their prefetch tasks
        for url in removed {
        ...
        // Selection repair
        if let oldURL = oldSelectedURL, newURLs.contains(oldURL) {
            // Old selection still exists — restore by matching imageURL (UUID was regenerated)
            if let restoredPair = pairs.first(where: { $0.imageURL == oldURL }) {
                selectedID = restoredPair.id
                detailID = restoredPair.id
            }
        } else if let oldIdx = oldSelectedIndex {
            // Old selection was deleted — jump to neighbor at same index (clamped)
            if pairs.isEmpty {
                selectedID = nil
                detailID = nil
            } else {
                let clampedIdx = min(oldIdx, pairs.count - 1)
                selectedID = pairs[clampedIdx].id
                detailID = pairs[clampedIdx].id
            }
            // Silently discard edits for deleted file
            _editingIsDirty = false
        }

        // Re-trigger prefetch around current selection
        if let id = selectedID {
            triggerPrefetch(aroundID: id)
        }

        // Rebuild folder tree so new subfolders under content dir appear
        if let root = rootDirectoryURL {
            folderTree = buildFolderTree(from: root)
        }

        print("[watchdog] content rescan: +\(added.count) -\(removed.count) ~\(surviving.count) surviving evicted")
    }
