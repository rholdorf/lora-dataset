...
struct ZoomablePannableImage: NSViewRepresentable {
    ...
    func updateNSView(_ nsView: ZoomableImageNSView, context: Context) {
        ...
        if nsView.image !== image {
            nsView.image = image
            // Bug (3) fix: resetToFit may be a no-op if bounds are zero at this
            // point (the NSView hasn't been laid out yet).  ZoomableImageNSView
            // tracks this with needsFit and retries in viewDidLayout / draw.
            nsView.resetToFit()
...
final class ZoomableImageNSView: NSView {
    ...
    // Bug (3) fix: when resetToFit() is called before the view has been given
    // its final bounds (bounds.width == 0), record that we still need to fit
    // so we can retry once layout completes.
    private var needsFit: Bool = false
    ...
    // Bug (3) fix: viewDidLayout is called after the view's bounds are final.
    // If a fit was requested when bounds were zero, apply it now.
    override func layout() {
        super.layout()
        if needsFit {
            resetToFit()
            // If bounds are now valid, the fit will succeed and clear needsFit.
            // Propagate new scale/offset back to SwiftUI bindings.
            if !needsFit {
                coordinator?.updateBindings(scale: scale, offset: offset)
                needsDisplay = true
            }
        }
    }
    ...
    public func resetToFit() {
        ...
        if iw > 0 && ih > 0 && bounds.width > 0 && bounds.height > 0 {
            ...
            // Fit succeeded — clear deferred flag
            needsFit = false
            needsDisplay = true
        } else {
            // Bounds not ready yet — request a deferred fit at next layout pass
            needsFit = true
        }
    }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        // Bug (3) fix: if a deferred fit is still pending (layout hasn't
        // completed yet at this draw call), apply it now so the image isn't
        // rendered at scale=1 (pixel-size) before the fit fires.
